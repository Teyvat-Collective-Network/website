...
<div class="container">
    <Navigation last={["/info/quickstart#global", "Quickstart"]}>
        ...
        <ul>
            <li><b><code>general</code>:</b> A public "general" channel which can be made accessible to all server members.</li>
            <li>
                <b><code>lounge</code>:</b> The staff general channel which can be made accessible to anyone you would trust with access to your server's general
                staff chat.
            </li>
            <li><b><code>office</code>:</b> The staff business channel which can be made accessible to your current staff.</li>
        </ul>
        ...
        <p>Once your channel permissions are set up, use <code>/connect</code> and select the appropriate global channel.</p>
        ...
        <p>To disconnect your channel, use <code>/disconnect</code> and the bot will immediately stop relaying messages in both directions.</p>
        ...
        <p>If the deletion didn't work or was incomplete, you can use <code>/purge</code> to instruct the bot to retry recent purges.</p>
        <Linkable e="h3" id="banning">Banning</Linkable>
        <p>
            Use <code>/ban</code> to ban a user from the current global channel or right click a message and select <code>Apps &gt; Ban Author</code>. This
            makes the bot stop relaying messages from that user across all servers. The bot will also automatically delete any messages they send in any
            connected channels.
        </p>
        <p>To use this command, you must have the <b>Ban Members</b> permission or be a designated global chat mod.</p>
        <p>
        ...
        <p>
            To get the ID of the author of a message, right click the message and select <code>Apps &gt; Get Author ID</code> or run <code>/get-author</code> with
            URL of any copy of the message.
        </p>
        ...
        <p>
            If there is a raid or ongoing network-wide incident, you can use <code>/panic</code> to completely shut down this global channel. It will stop relaying
            messages from all servers but will continue to relay deletions.
        </p>
        <Linkable e="h3" id="for-observers">For Observers</Linkable>
        <ul>
            <li><code>/globals list</code> &mdash; List all global channels.</li>
            <li>
                <code>/globals delete</code> &mdash; Delete a global channel. Do <b>not</b> use this unless the council has agreed to terminate a global channel
                or it was a temporary channel that is no longer needed.
            </li>
            <li><code>/globals create</code> &mdash; Create a global channel.</li>
            <li><code>/globals edit</code> &mdash; Open the edit screen for a global channel.</li>
        </ul>
        <Linkable e="h3" id="other-commands">Other Commands</Linkable>
        <ul>
            <li>
                <code>/faq</code> &mdash; Show information about the bot. If someone asks what global chat is or why everyone is a bot (fairly common), use this
                command with <code>public: true</code> to show the info page (it will be shown in all servers).
            </li>
            <li>
                <code>/nickname</code> &mdash; Set your global nickname. By default, your display name (nickname or username) will be shown to other servers. Setting
                this option will override your display name in all global channels from all servers.
            </li>
            <li>
                <code>/metrics</code> &mdash; If you think the bot is lagging or not responding, you can try running this command to check some bot metrics.
            </li>
            <li>
                <code>/rules</code> &mdash; Show the global chat rules. Individual server rules still apply and moderation may be done by individual servers'
                staff, but a baseline set of rules exists. Use <code>public: true</code> to show this publicly (it will be shown in all servers).
            </li>
        </ul>
    </Navigation>
</div>
